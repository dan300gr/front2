...
const StockList = () => {
  ...
  const fetchProductos = async () => {
    try {
      const response = await axios.get('https://api2-uetw.onrender.com/api/productos/'); // Asegúrate de que este endpoint sea correcto
  ...
  const handleDelete = async (stock_id) => {
    try {
      ...
      await axios.delete(`https://api2-uetw.onrender.com/api/stocks/${stock_id}`);
      Swal.fire({
        icon: 'success',
