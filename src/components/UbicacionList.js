...
const UbicacionList = () => {
  ...
  const handleDelete = async (ubicacion_id) => {
    try {
      ...
      const inventarioResponse = await axios.get('https://api2-uetw.onrender.com/api/inventarios/');
      ...
      await axios.delete(`https://api2-uetw.onrender.com/api/ubicaciones/${ubicacion_id}`);
      Swal.fire({
        icon: 'success',
